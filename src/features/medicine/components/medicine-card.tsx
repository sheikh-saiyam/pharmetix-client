  ...
  return (
    <Link href={`/medicines/${medicine.slug}`}>
      <Card
        ...
        className="group relative h-full flex flex-col border-slate-100 bg-white hover:border-(--brand-primary) hover:shadow-lg hover:shadow-(--brand-primary)/5 transition-all duration-300 pt-0 pb-4 overflow-hidden"
      >
        {/* Image Section */}
        ...
        <CardContent className="flex flex-col flex-1 px-5 py-0">
          <div className="mb-2">
            <p className="text-[12px] font-bold text-(--brand-primary) mb-1 group-hover:text-slate-800">
              {medicine.manufacturer}
            </p>
            <h3 className="font-bold text-slate-800 text-lg leading-tight group-hover:text-(--brand-primary) transition-colors line-clamp-1">
              {medicine.brandName}
            </h3>
            ...
            <div className="flex gap-2">
              {canAddToCart && !isOutOfStock && (
                <Button
                  ...
                  className="h-9 w-9 rounded-full hover:opacity-90 shadow-xs transition-all active:scale-95"
                  onClick={(e) => {
                    e.preventDefault(); // Safety
                    e.stopPropagation(); // CRITICAL: Stops Link navigation
                    addItem(medicine);
                  }}
                >
